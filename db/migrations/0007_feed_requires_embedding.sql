-- Six Double Crisis cards (dc1-1, dc1-10..dc1-14) are in TCGdex's catalog with an
-- image URL that 404s, so they ingest fine and then render as broken cards. The
-- manifest cannot know a URL is dead, and load_db follows the manifest.
--
-- Rather than deleting those six and waiting for the next set to reintroduce the
-- problem, the feed now refuses to serve any card without an embedding. No art
-- means nothing to embed, so "has an embedding" is the same condition as "has art
-- we actually hold", and it is checked in one place instead of at every call site.
-- It also keeps null vectors out of the similarity math entirely.

create or replace function public.feed_for_user(p_limit int default 20)
returns table (
